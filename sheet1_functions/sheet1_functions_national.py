...
def create_sheet1(complete_data, metadata, output_dir, global_data):
    ...
    in_situ_data = metadata['insitu_files']
    
    if in_situ_data:
        inflow_values, inflow_dates = sum_ts([in_situ_data[0]])
        outflow_values, outflow_dates = sum_ts([in_situ_data[1]])
        transfer_values, transfer_dates = sum_ts([in_situ_data[2]])                
    else:
    ...
    if in_situ_data:
        common_dates = becgis.common_dates([complete_data['p'][1], complete_data['et'][1], outflow_dates, inflow_dates])
    else:
        common_dates = becgis.common_dates([complete_data['tr'][1],complete_data['p'][1], complete_data['et'][1]]) #, outflow_dates])
    ...
    for date in common_dates:
        ...
        entries = { 'P': complete_data['p'][0][complete_data['p'][1] == date][0],
        ...
        results = calc_sheet1(entries, metadata['lu'], sheet1_lucs, metadata['recycling_ratio'], q_outflow, output_folder,
...
#    utilizable_outflow = available_water - utilized_flow
    ...
    non_cons_water = exploitable_water - utilized_flow
...
def calc_sheet1(entries, lu_fh, sheet1_lucs, recycling_ratio, q_outflow, 
    ...
    ET = becgis.open_as_array(entries['ET'], nan_values = True) 
#    ETgreen = becgis.open_as_array(entries['ETgreen'], nan_values = True)
#    ETblue = becgis.open_as_array(entries['ETblue'], nan_values = True)
    
    #Calculate Peffective based on FAO lookup table http://www.fao.org/3/S2022E/s2022e03.htm
    # A model is to be prefered for ETb/ETg seperation if rainfall is a significant source of water for agriculture.
    ####
    Peff = (P*0 + np.logical_and(P>=10, P<=20)*(0.2*P-2) + np.logical_and(P>20, P<=70)*(0.6*P-10) +
            np.logical_and(P>70, P<=80)*(0.7*P-17) + (P>80)*(0.8*P-25))
    #####
    
    ETgreen = (Peff<=ET)*Peff + (Peff>ET)*ET
    ETblue = ET - ETgreen
    
    pixel_area = becgis.map_pixel_area_km(lu_fh)
   
    P[np.isnan(LULC)] = ETgreen[np.isnan(LULC)] = ETblue[np.isnan(LULC)] = ET[np.isnan(LULC)] = np.nan
    P, ETgreen, ETblue, ET = np.array([P, ETgreen, ETblue, ET]) * 0.000001 * pixel_area
    
#    ET = np.nansum([ETblue, ETgreen], axis = 0)
    
    results['et_advection'], results['p_advection'], results['dS'] = calc_wb(P, ET, q_outflow, 
...
#    net_inflow = results['p_advection'] + q_in_sw + q_in_gw + q_in_desal + results['dS'] 
#    consumed_water = np.nansum(list(landscape_et.values())) + np.nansum(list(incremental_et.values()))
    non_consumed_water = q_outflow
...
#    results['reserved_outflow_actual'] = min(non_consumed_water - results['non_utilizable_outflow'], results['reserved_outflow_demand'])
    results['utilizable_outflow'] = non_consumed_water
    ...
    results['uf_mwu'] = incremental_et['Managed']
    results['uf_mlu'] = incremental_et['Modified']
    results['uf_ulu'] = incremental_et['Utilized']
    results['uf_plu'] = incremental_et['Protected']
...
def calc_wb(P, ET, q_outflow, q_in_sw = 0, q_in_gw = 0, q_in_desal = 0, q_out_sw = 0, q_out_gw = 0):
    """
...
def sum_ts(flow_csvs):
    ...
    for cv in flow_csvs:     
        coordinates, flow_ts, station_name, unit = create_dict_entry(cv)
